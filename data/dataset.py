...
class Text2MotionDatasetMotionx(data.Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer):
        ...
        for name in tqdm(id_list):
            try:
                motion = np.load(pjoin(opt.motion_dir, name + '.npy'))
                if (len(motion)) < min_motion_len or (len(motion) >= self.max_motion_length+4):
                    continue
                text_data = []
                flag = False
                with cs.open(pjoin(opt.text_dir, name + '.txt')) as f:
                    for line in f.readlines():
                        ...
                        line_split = line.strip().split('#')
                        # if len(line_split)==1:
                        #     line_split.extend(['a/DET person/NOUN', 0.0 , 0.0])
                        ...
                        if f_tag == 0.0 and to_tag == 0.0:
                            ...
                            try:
                                n_motion = motion[int(f_tag*20) : int(to_tag*20)]
                                if (len(n_motion)) < min_motion_len or (len(n_motion) >= self.max_motion_length+4):
                                    continue
                                new_name = random.choice('ABCDEFGHIJKLMNOPQRSTUVW') + '_' + name
                                while new_name in data_dict:
                                    new_name = random.choice('ABCDEFGHIJKLMNOPQRSTUVW') + '_' + name
