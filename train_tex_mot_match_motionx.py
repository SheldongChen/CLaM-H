...
from data.dataset import Text2MotionDatasetMotionx, collate_fn
...
if __name__ == '__main__':
    ...
    if opt.dataset_name == 't2m':
        ...
        opt.motion_dir = pjoin(opt.data_root, 'new_joint_vecs')
        opt.text_dir = pjoin(opt.data_root, 'texts')
        opt.joints_num = 52 # 22 + 30
        opt.max_motion_length = 196 #440 #196
        dim_pose = 623
        num_classes = 200 // opt.unit_length
        meta_root = pjoin(opt.checkpoints_dir, opt.dataset_name, 'Decomp_SP001_SM001_H512', 'meta')
    ...
    train_dataset = Text2MotionDatasetMotionx(opt, mean, std, train_split_file, w_vectorizer)
    val_dataset = Text2MotionDatasetMotionx(opt, mean, std, val_split_file, w_vectorizer)
